...
from data_processing.dataset_configuration import get_dataset_info, get_dataset_obj, get_datasets, get_datasets
...
def get_simple_attack_configuration(dataset):
    data = dict()
...
def simple_backdoor(func):
    @wraps(func)
    def wrapper(*args, **kwargs):       
        if not general_args.is_attack or general_args.backdoor_method != 'simple':
            return func(*args, **kwargs)
        logger.warning_once(f"⚠️ simple backdoor is activated for function {func.__name__} in {args[0].__class__.__name__}")
        ...
        if func.__name__ == '_net_eval' and args[0].__class__.__name__ == 'DataConsumer':
            ...
            attack_info = get_simple_attack_configuration(general_args.dataset)
            ...
            _self, *_ = args
            logger.warning_once(f"⚠️ simple backdoor is activated for {general_args.dataset} dataset.")
        ...
        # init backdoor trigger
            ...
            _self.trigger, _, _self.mask = get_trigger(**trigger_config)
