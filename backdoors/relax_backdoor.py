...
SIMPLE_TRANS = 0.3 # this parameter directly affects the ASR
trigger_config = {
    'trigger_type': 'wholeimage',
    'label_type': 'random',
    'img_size': get_dataset_info(general_args.dataset)['img_size'][0],
}

'''Set the Trigger Pattern'''
def get_trigger(trigger_type='wholeimage', label_type='random', img_size=32, num_classes=10, normalization=False):

    channel_means = get_dataset_info(general_args.dataset)['mean']
    channel_stds = get_dataset_info(general_args.dataset)['std']
    trigger_shape = np.array([1, img_size, img_size, len(channel_means)])
    ...
    # convert to tensor
    trigger = torch.tensor(trigger, dtype=torch.float32).to(general_args.device)
    trigger_label = torch.tensor(trigger_label, dtype=torch.long).to(general_args.device)
    mask = torch.tensor(mask, dtype=torch.float32).to(general_args.device)

    # logger.info(f"trigger.shape = {trigger.shape}")
    trigger = trigger.permute(0, 3, 1, 2)
    mask = mask.permute(0, 3, 1, 2)

    # # load trigger from npy 
    # trigger = np.load('/home/user009/CODE/FedDOGE/backdoors/learned_trigger.npy')
    # trigger = torch.tensor(trigger, dtype=torch.float32).to(general_args.device)
    # trigger = trigger.permute(0, 3, 1, 2)
...
def get_relax_attack_configuration(dataset):
    data = dict()
...
def relax_backdoor(func):
    @wraps(func)
    def wrapper(*args, **kwargs):       
        if not general_args.is_attack or general_args.backdoor_method != 'relax':
            return func(*args, **kwargs)
        logger.warning_once(f"⚠️ relax backdoor is activated for function {func.__name__} in {args[0].__class__.__name__}")
        ...
        if func.__name__ == '_net_eval' and args[0].__class__.__name__ == 'DataConsumer':
            ...
            attack_info = get_relax_attack_configuration(general_args.dataset)
            ...
            if general_args.dataset in ['cifar10', 'tiny-imagenet']:
                GLOBAL_MASK = ckpt['mask'].to(device).permute(1, 2, 0)
                GLOBAL_TRIGGER = ckpt['trigger'].to(device).permute(1, 2, 0)
            else:
                GLOBAL_MASK = ckpt['mask'].to(device)
                GLOBAL_TRIGGER = ckpt['trigger'].to(device)
            GLOBAL_TRIGGER = (GLOBAL_TRIGGER * 255).clamp(0, 255).round()
            ...
            if type(poisoned_testset.data) == torch.Tensor:
                poisoned_testset.data = poisoned_testset.data.cpu().numpy()

            poisoned_testset.data = poisoned_testset.data * (1-GLOBAL_MASK.cpu().numpy()) + GLOBAL_MASK.cpu().numpy() * GLOBAL_TRIGGER.cpu().numpy()
            poisoned_testset.data = poisoned_testset.data.round().clip(0, 255).astype(np.uint8)
            ...
            _self, *_ = args
            logger.warning_once(f"⚠️ relax backdoor is activated for {general_args.dataset} dataset.")
            dataset_length = get_dataset_info(general_args.dataset)['dataset_length']
            # samples with backdoor
            _self.perm = np.random.permutation(dataset_length)[0: int(dataset_length * 0.1)]
        ...
        # init backdoor trigger
            ...
            from third_party.KIP.relax import generate_trigger_from_synthetic_images
            ...
            result0_trim, result1_trim = func(*args,cache=True, **kwargs)
            logger.info(f"Secondly, we will generate the trigger.")
            _, _, _self.mask = get_trigger(**trigger_config)
            
            np_trigger = generate_trigger_from_synthetic_images(results_trim0=result0_trim, results_trim1=result1_trim, logger=logger,dataset_info=get_dataset_info(general_args.dataset))
            ...
            _self.mask = _self.mask * SIMPLE_TRANS
            logger.info(f"✨Done!")
            logger.info(f"Thirdly, we will use the trigger to synthesic.")
            ...
            # # (1 - mask_rate) * array + mask_rate * trigger
            # logger.info(F"MASK.SHAPES: {_self.mask.shape}, _self.trigger.shape: {_self.trigger.shape}")

            # _self.images_with_trigger[_self.perm] = (1 - _self.mask) * _self.images_with_trigger[_self.perm] + _self.mask * _self.trigger
            ...
            # =======================
            # New Code for Sampling and Plotting
            # =======================
            # Randomly sample 100 images
            sample_size = 100
            total_images = _self.images_with_trigger.size(0)
            if total_images < sample_size:
                raise ValueError(f"Not enough images to sample. Required: {sample_size}, Available: {total_images}")
            
            # Generate random indices without replacement
            sample_indices = random.sample(range(total_images), sample_size)
            sample_images = _self.images_with_trigger[sample_indices].detach().cpu()

            # De-normalize images if they were normalized
            dataset_info = get_dataset_info(general_args.dataset)
            if general_args.dataset in ['mnist', 'fmnist']:
                mean = torch.tensor(dataset_info['mean']).view(1, 1, 1, 1)
                std = torch.tensor(dataset_info['std']).view(1, 1, 1, 1)
            else:
                mean = torch.tensor(dataset_info['mean']).view(1, 3, 1, 1)
                std = torch.tensor(dataset_info['std']).view(1, 3, 1, 1)
            sample_images = sample_images * std + mean  # Assuming normalization was (x - mean) / std
            sample_images = torch.clamp(sample_images, 0, 1)  # Ensure the pixel values are within [0,1]

            # Convert tensor to NumPy for plotting
            sample_images = sample_images.permute(0, 2, 3, 1).numpy()  # Change to (N, H, W, C)

            # Plot images using matplotlib
            grid_size = 10  # 10x10 grid
            fig, axes = plt.subplots(grid_size, grid_size, figsize=(20, 20))
            fig.suptitle('Randomly Sampled Images with Trigger', fontsize=16)
            ...
            plt.show()
            plt.savefig(os.path.join(TRIGGER_FILE_PATH,f"{start_time[1]:02}{start_time[2]:02}{start_time[3]:02}{start_time[4]:02}{start_time[5]:02}",'sampled_images.png'))
            ...
            C, H, W = _self.channel, _self.img_size[0], _self.img_size[1]
            ...
            _self.images_all = [torch.unsqueeze(train_set[i][0], dim=0) for i in local_data]
            labels_all = torch.tensor([train_set[i][1] for i in local_data], dtype=torch.long, device=_self.device)
            for i, lab in enumerate(labels_all):
                _self.indices_class[lab].append(i)
            _self.images_all = torch.cat(_self.images_all, dim=0).to(_self.device)
            ...
            # which class can be utilized to generate synthesized images
            _self.trainable_classes = [c for c in range(_self.num_classes)
                                       if len(_self.indices_class[c]) >= _self.idm_threshold]
            ...
            # which class can be utilized to generate synthesized images
            _self.trainable_classes = [c for c in range(_self.num_classes)
                                    if len(_self.indices_class[c]) >= _self.dam_threshold]
            ...
            if general_args.init == 'real':
                logger.info('initialize synthetic data from random real images')
                for c in range(_self.num_classes):
                    image_syn.data[c*general_args.ipc:(c+1)*general_args.ipc] = get_images(c, general_args.ipc, _self.images_all).detach().data
            ...
            max_mean = 0
            for it in range(general_args.iteration+1):
                ''' Train synthetic data '''
                net = get_network(general_args.synthesis_model, channel, _self.num_classes, im_size).to(general_args.device) # get a random model
                net.train()
                for param in list(net.parameters()):
