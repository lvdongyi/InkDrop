...
from synthesis_methods.cafe.cafe_networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP

GLOBAL_DATASET = None
def set_global_dataset(dataset):
...
def criterion_middle(real_feature, syn_feature):
    global GLOBAL_DATASET
    nc = 10
    if GLOBAL_DATASET in ['tiny-imagenet', 'tiny', 'tiny_imagenet']:
        nc = 1
    MSE_Loss = nn.MSELoss(reduction='sum')
    shape_real = real_feature.shape
    real_feature = torch.mean(real_feature.view(nc, shape_real[0] // nc, *shape_real[1:]), dim=1)

    shape_syn = syn_feature.shape
    syn_feature = torch.mean(syn_feature.view(nc, shape_syn[0] // nc, *shape_syn[1:]), dim=1)
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if output.dtype == torch.float32:
            ...
            acc = np.sum(np.equal(np.argmax(output.detach().cpu().to(torch.float32).numpy(), axis=-1), lab.detach().cpu().numpy().astype(np.int64)))
