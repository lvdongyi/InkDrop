...
from .dc_networks import MLP, ConvNet, LeNet, AlexNet, AlexNetBN, VGG11, VGG11BN, ResNet18, ResNet18BN_AP, ResNet18BN
...
#         std = [0.3530]
#         transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
#         dst_train = MNIST.FashionMNIST(data_path, train=True, download=True, transform=transform) # no augmentation
#         dst_test = MNIST.FashionMNIST(data_path, train=False, download=True, transform=transform)
...
#         data = torch.load(os.path.join(data_path, 'tinyimagenet.pt'), map_location='cpu',weights_only=True)
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    if model == 'MLP':
        ...
        net = ConvNet(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth,
                        net_act=net_act, net_norm='maskbatchnorm', net_pooling=net_pooling, im_size=im_size)
    elif model == 'LeNet':
        net = LeNet(channel=channel, num_classes=num_classes)
    elif model == 'AlexNet':
        net = AlexNet(channel=channel, num_classes=num_classes)
    elif model == 'AlexNetBN':
        net = AlexNetBN(channel=channel, num_classes=num_classes, image_size=im_size)
    elif model == 'VGG11':
        net = VGG11( channel=channel, num_classes=num_classes, image_size=im_size)
    elif model == 'VGG11BN':
