...
from torchvision.datasets.utils import download_and_extract_archive, verify_str_arg, check_integrity
...
class STL10(VisionDataset):
    ...
    def __init__(
        ...
        if resize:
            self.data = np.array([self._resize_image(img) for img in self.data])
    ...
    def _resize_image(self, img: np.ndarray) -> np.ndarray:
        ...
        """
        img_pil = Image.fromarray(np.transpose(img, (1, 2, 0)))  # Convert to PIL Image
        img_resized = img_pil.resize((32, 32), Image.Resampling.LANCZOS)  # Resize to 32x32 using LANCZOS filter
        img_resized = np.transpose(np.array(img_resized), (2, 0, 1))  # Convert back to ndarray
        return img_resized
